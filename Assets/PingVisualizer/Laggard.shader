Shader "Custom/UnlitVR_Opacity_Fresnel"
{
    Properties
    {
        _MainTex ("Texture", 2D) = "white" {}
        _Color ("Tint Color", Color) = (1,1,1,1)
        _Opacity ("Inside Opacity", Range(0,1)) = 1
        ...
        _Inflate ("Vertex Inflate Amount", Range(0,0.1)) = 0.005
    }
    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            float4 frag(Varyings IN) : SV_Target
            {
                ...
                float isRed   = step(0.99, IN.color.r) * step(IN.color.g, 0.01) * step(IN.color.b, 0.01);
                float isGreen = step(0.99, IN.color.g) * step(IN.color.r, 0.01) * step(IN.color.b, 0.01);

                float isHead = saturate(isRed + isGreen);

                float headAlpha = lerp(1.0, _HeadOpacity, isHead);

                finalColor.a = baseAlpha * headAlpha * _BodyOpacity;
